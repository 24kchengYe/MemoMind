...
| Feature | MemoMind | Mem0 | Graphiti/Zep | Letta | Cognee | Hindsight |
|---------|----------|------|--------------|-------|--------|-----------|
| **GitHub Stars** | — | 51K | 24K / 4K | 22K | 15K | 7K |
| **Funding** | Self-funded | $24M (YC) | — | $10M | $7.5M | $3.5M |
| **Architecture** | KG + pgvector | Vector + Graph | Temporal KG | Agent OS | ECL + KG | 4-network |
| **Retrieval** | 4-way hybrid | Semantic + graph | Sem+BM25+graph | Agent-driven | 14 modes | 4 parallel |
| **Knowledge Graph** | Built-in (pgvector) | Pro only ($249/mo) | Core (Neo4j) | No | Yes | Yes |
| **Temporal** | Native | No | Bi-temporal | No | Partial | Yes |
| **Privacy** | 100% local | Cloud default | Cloud/BYOC | Self-host opt | Local/Cloud | Local |
| **GPU Accel** | Local CUDA | No | No | No | No | No |
| **LongMemEval** | — | 49% | — | — | — | 91.4% |
| **Cost** | $0.30/mo | Free–$249/mo | Free–$475/mo | Free–$200/mo | Free–$200/mo | Free (OSS) |
...
- [x] MinerU integration — GPU-accelerated PDF/DOCX/PPTX parsing with formula, table, image extraction
- [x] Full-disk scanning — auto-discover all documents across D/G/H drives
- [x] Unified card template v2 — Language, Category, Keywords, Summary fields
...
- **v1.8** (2026-04-12): **MinerU document parsing engine** — replaced pdfplumber/python-docx/python-pptx with [MinerU](https://github.com/opendatalab/MinerU) 3.0.9 as default extraction engine; GPU-accelerated (RTX 3070, CUDA 12.6) layout analysis, formula recognition (LaTeX $$...$$), table structure preservation (HTML `<table>`), image extraction with captions; full-disk scanning (D/G/H drives, auto-categorize); unified card template v2 with Language/Category/Keywords/Summary fields; MinerU API auto-start/stop lifecycle (port 8010, zero idle resource usage); `--nice` mode for background operation; legacy fallback when MinerU unavailable; 100% local processing — all models run on-device, zero cloud API calls
...
| 特性 | MemoMind | Mem0 | Graphiti/Zep | Letta | Cognee | Hindsight |
|------|----------|------|--------------|-------|--------|-----------|
| **GitHub Stars** | — | 51K | 24K / 4K | 22K | 15K | 7K |
| **融资** | 自筹 | $24M (YC) | — | $10M | $7.5M | $3.5M |
| **架构** | KG + pgvector | 向量 + 图 | 时序 KG | Agent OS | ECL + KG | 4 网络 |
| **检索** | 4 路混合 | 语义 + 图 | 语义+BM25+图 | Agent 驱动 | 14 种模式 | 4 路并行 |
| **知识图谱** | 内置（pgvector） | Pro 版（$249/月） | 核心（Neo4j） | 无 | 有 | 有 |
| **时序能力** | 原生 | 无 | 双时态 | 无 | 部分 | 有 |
| **隐私** | 100% 本地 | 默认云端 | 云/自部署 | 可自托管 | 本地/云 | 本地 |
| **GPU 加速** | 本地 CUDA | 无 | 无 | 无 | 无 | 无 |
| **LongMemEval** | — | 49% | — | — | — | 91.4% |
| **成本** | ¥2/月 | 免费–$249/月 | 免费–$475/月 | 免费–$200/月 | 免费–$200/月 | 免费 (OSS) |
...
- **v1.8** (2026-04-12): **MinerU 文档解析引擎**——用 [MinerU](https://github.com/opendatalab/MinerU) 3.0.9 替代 pdfplumber/python-docx/python-pptx；GPU 加速（RTX 3070, CUDA 12.6）版面分析、公式识别（LaTeX $$...$$）、表格结构保留（HTML `<table>`）、图片提取含标注；全盘扫描（D/G/H 盘自动分类）；统一卡片模板 v2（Language/Category/Keywords/Summary 字段）；MinerU API 随用随启（端口 8010，空闲零资源占用）；`--nice` 后台低优先级模式；100% 本地处理——所有模型在本机推理，零云端调用
