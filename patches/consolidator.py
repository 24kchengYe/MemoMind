...
async def _process_memory_batch(
    ...
    for create in llm_result.creates:
        ...
        # When proof_count would be 1 and the observation text is >80% similar to the source, skip it.
        # Real observations should synthesize across multiple facts or provide genuine new insight.
        if len(source_mems) == 1:
            _src_text = (source_mems[0].get("text") or "").strip().lower()
            _obs_text = create.text.strip().lower()
            # Strip metadata suffixes for comparison
            for _sep in ["| involving:", "| when:", "| 博士论"]:
                if _sep in _src_text: _src_text = _src_text.split(_sep)[0].strip()
            # Check similarity: if one contains the other or they share >80% characters
            ...
            if _shorter > 10 and _longer > 0:
                ...
                if _similarity > 0.5 or (_src_text in _obs_text) or (_obs_text in _src_text) or (_shorter / _longer > 0.6):
                    continue  # Skip trivial observation
