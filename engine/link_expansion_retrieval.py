...
class LinkExpansionRetriever(GraphRetriever):
    ...
    async def _expand_combined(
        ...
        all_rows = await conn.fetch(
            f"""
            WITH super_entities AS (
                -- Exclude super-hub entities (mention_count > 200) that add noise
                -- to graph traversal (e.g., "用户", "user" appear in nearly every memory)
                SELECT id FROM {fq_table("entities")}
                WHERE mention_count > 200
            ),
            entity_expanded AS (
                ...
                -- Filters out super-hub entities to prevent noise flooding.
                SELECT
                ...
                WHERE ml.from_unit_id = ANY($1::uuid[])
                  ...
                  AND ml.entity_id NOT IN (SELECT id FROM super_entities)
                GROUP BY mu.id
                ORDER BY score DESC
                LIMIT $3
            ),
