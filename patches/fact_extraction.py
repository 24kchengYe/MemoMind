    ...
    for attempt in range(llm_max_retries):
        try:
            ...
            for i, llm_fact in enumerate(raw_facts):
                ...
                # PATCH: If occurred_start not set by LLM, fallback to event_date
                if not fact_data.get("occurred_start") and event_date is not None:
                    fact_data["occurred_start"] = event_date.isoformat()
                    if not fact_data.get("occurred_end"):
                        fact_data["occurred_end"] = event_date.isoformat()
